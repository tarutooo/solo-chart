document.addEventListener('DOMContentLoaded', function() {
    ...
    function initTabFunctionality() {
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                // メモタブがアクティブになった時にメモをフォーカス
                if (targetTab === 'memo') {
                    setTimeout(() => {
                        const memo = document.getElementById('project-memo');
                        if (memo) memo.focus();
                    }, 50);
                }
            });
    ...
    function initMemoTab() {
        const memoEl = document.getElementById('project-memo');
        const savedIndicator = document.getElementById('memo-saved-indicator');
        const countEl = document.getElementById('memo-char-count');
        if (!memoEl) return; // メモタブがない場合は何もしない

        // 保存済みメモを読み込み
        try {
            const saved = localStorage.getItem('projectMemoV1');
            if (saved !== null) memoEl.value = saved;
        } catch (_) { /* no-op */ }

        // 文字数カウント表示
        const updateCount = () => {
            if (countEl) countEl.textContent = `${memoEl.value.length} 文字`;
        };
        updateCount();

        const doSave = () => {
            try {
                localStorage.setItem('projectMemoV1', memoEl.value);
                if (savedIndicator) savedIndicator.textContent = '保存済み';
                if (savedIndicator?.parentElement) savedIndicator.parentElement.classList.remove('saving');
            } catch (_) {
                if (savedIndicator) savedIndicator.textContent = '保存エラー';
            }
        };

        const debouncedSave = debounce(doSave, 300);

        memoEl.addEventListener('input', () => {
            if (savedIndicator) savedIndicator.textContent = '保存中…';
            if (savedIndicator?.parentElement) savedIndicator.parentElement.classList.add('saving');
            updateCount();
            debouncedSave();
        });
